            ...
            if (cooked) {
                if (cooked_type == "MSCZ" || cooked_type == "MSC") {
                    if (id == resourceId.sha256()!) {
                        ...
                        iReader.readInt32()
                        response?.raw_type = NSString(data: NSData(base64EncodedString: readString(iReader), options: NSDataBase64DecodingOptions.allZeros)!.decrypt(Cipher.ChaCha20Enum(setup))!, encoding: NSUTF8StringEncoding)!
                        response?.raw_length = readInt(iReader)
                        response?.cooked_length = readInt(iReader)
                        response?.data = readData(iReader).decrypt(Cipher.ChaCha20Enum(setup))!
                        if (cooked_type == "MSCZ") {
